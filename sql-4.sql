...
select replace(firstname,'a','A'),firstname from worker2;
select firstname,department
from
(select *,rank() over(partition by department order by firstname asc) from worker2)
as tm order by department desc;
